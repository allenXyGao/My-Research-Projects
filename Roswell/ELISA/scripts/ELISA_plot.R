# The two text files contain the concentration values.
# The excel file contains sample information. Please use the "qryPIyaoSamples (2)" sheet.
rm(list=ls())
library(readxl)
library(dplyr)
library(ggplot2)
...
filepath.concentration.540nm <- "/projects/rpci/qzhu/xinyu/mywork/ELISA_par4/inputs/ELISA_450nm-540nm"
filepath.concentration.570nm <- "/projects/rpci/qzhu/xinyu/mywork/ELISA_par4/inputs/ELISA_450nm-570nm"
filepath.sample.info <- "/projects/rpci/qzhu/xinyu/mywork/ELISA_par4/inputs/ELISA_samples_barcode.xlsx"

# sample info
sample.info <- read_excel(path = filepath.sample.info, sheet = "qryPIyaoSamples (2)")
...
# BARCODE is the column that will be used to merge with concentration data.

###############################################################################################################################
# 540 nm
concen.540 <- read.table(file = filepath.concentration.540nm, header = TRUE)
colnames(concen.540)[1] <- "BARCODE"
dat.540 <- merge(concen.540, sample.info.dat, by="BARCODE")

table(dat.540$TXHISTORYSTATUS)
# No prior treatment (excluding diagnostic biopsy)    NO PRIOR TREATMENT (EXCLUDING DIAGNOSTIC BIOPSY) 
#                      9                                                   4 
# Post-surgical AND Post-adjuvant or systemic therapy       Post-surgical/No adjuvant or systemic therapy 
#                      6                                                   4 
...
dat.540$TXHISTORYSTATUS[dat.540$TXHISTORYSTATUS == "Post-surgical/No adjuvant or systemic therapy"] <- "Post but no-adjuvant"
...
mycols <- c("cadetblue", "chocolate1", "darkgreen" , "gold3", "darkorchid2", "gray70")
for (j in 1:dim(patients.variant.info)[1]) {
  ...
  if (dim(cur.patient.dat)[1] < 3) {segments(x0 = cur.patient.dat[1,1], y0 = cur.patient.dat[1,2],
                                             x1 = cur.patient.dat[2,1], y1 = cur.patient.dat[2,2], col = mycols[j])}
  else {
    for (t in 1:(dim(cur.patient.dat)[1] -1)) {
      segments(x0 = cur.patient.dat[t,1], y0 = cur.patient.dat[t,2],
               x1 = cur.patient.dat[t+1,1], y1 = cur.patient.dat[t+1,2], col = mycols[j])
    }
  }
}
...
# 6 PT-00195528     3
# mycols <- c("cadetblue", "chocolate1", "darkgreen" , "gold3", "darkorchid2", "gray70")
# connect subjectID and cols
patients.variant.info$col <- mycols
out.subject.cols <- patients.variant.info 
filepath.out <- "/projects/rpci/qzhu/xinyu/mywork/ELISA_par4/outputs/plot_results_data/"
save(out.subject.cols, file=paste0(filepath.out,"1st_subject_colors.RData"))


###############################################################################################################################
# 570 nm
concen.570 <- read.table(file = filepath.concentration.570nm, header = TRUE)
...
abline(h=6, col="tomato3",  lty=2)
#segments(x0 = 1, y0 = 0, x1 = 5, y1 = 5.15, col = "darkgreen")
patients.variant.info <- dat.570 %>% group_by(SubjectID) %>% summarise(ct=n())
patients.variant.info <- patients.variant.info[patients.variant.info$ct > 1, ]
mycols <- c("cadetblue", "chocolate1", "darkgreen" , "gold3", "darkorchid2", "gray70")
for (j in 1:dim(patients.variant.info)[1]) {
  cur_ID <- as.character(patients.variant.info[j, 1])
  cur.patient.dat <- dat.570[dat.570$SubjectID == cur_ID, c("x_numbers", "conc.x")]
  if (dim(cur.patient.dat)[1] < 3) {segments(x0 = cur.patient.dat[1,1], y0 = cur.patient.dat[1,2],
                                             x1 = cur.patient.dat[2,1], y1 = cur.patient.dat[2,2], col = mycols[j])}
  else {
    for (t in 1:(dim(cur.patient.dat)[1] -1)) {
...
# store data
filepath.out <- "/projects/rpci/qzhu/xinyu/mywork/ELISA_par4/outputs/plot_results_data/"
save(dat.570, file=paste0(filepath.out,"1st_res.RData"))
