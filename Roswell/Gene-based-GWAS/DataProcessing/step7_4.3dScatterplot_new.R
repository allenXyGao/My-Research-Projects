...
for (i in 1:length(pop)){
  ...
  if (cohort.name == 'cohort1') {
    filepath <- paste("/projects/rpci/qzhu/xinyu/mywork/skatO_case_control/output/EigenStrat_Results.iter2/",
...
for (i in 1:length(pop)){
  ...
  cohort.name <- split.Full_name[2]
  if (cohort.name == 'cohort1') {
    s <- read.table(file=paste("/projects/rpci/qzhu/xinyu/mywork/skatO_case_control/output/EigenStrat_Results.iter2/", pop[i],
...
for (i in 1:length(pop)){
  ...
  cohort.name <- split.Full_name[2]
  if (cohort.name == 'cohort1') {
    filepath <- paste("/projects/rpci/qzhu/xinyu/mywork/skatO_case_control/output/EigenStrat_Results.iter2/", pop[i],
...
## r2 step7_1 only
## 0.009503656 0.005723215 0.012417999 0.007224144
## r2 sep7_1 + step7_2
## 0.010250078 0.005723215 0.011859439 0.007224144
...
this_3dplot.pc1 <- function(i) {
  ...
  cohort.name <- split.Full_name[2]
  if (cohort.name == 'cohort1') {
    dat1 <- read.table(file=paste("/projects/rpci/qzhu/xinyu/mywork/skatO_case_control/output/EigenStrat_Results.iter2/",
                                  pop[i], "/LDprune_autosome-pca1.eth.out", sep=""), header=T, as.is=T)
    dat2 <- read.table(file=paste("/projects/rpci/qzhu/xinyu/mywork/skatO_case_control/output/EigenStrat_Results.iter2/",
                                  pop[i], "/LDprune_autosome-pca2.eth.out", sep=""), header=T, as.is=T)
    xrange <- round(range(dat1[,3]), 1)
    yrange <- round(range(dat1[,4]), 1)
    zrange <- round(range(dat1[,5]), 1)
